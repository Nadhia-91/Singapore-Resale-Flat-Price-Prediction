...
def predict_price(year,town,flat_type,flr_area_sqm,flat_model,stry_start,stry_end,re_les_year,
    ...
    with open("/content/ResaleFlatPrices_Model.pkl","rb") as f:
...
selected = option_menu("Menu", ["About Project", "Price Predictions"])
...
if selected == "About Project":
    ...
    st.write("Data Wrangling, EDA, Model Building, Model Deployment")
    ...
    with col1:
        ...
        year= st.selectbox("Select the Year",["1990","1991","1992","1993","1994","1995","1996","1997","1999","2000","2001","2002","2003","2004","2005","2006","2007","2008","2009","2010","2011","2012","2013","2014","2015", "2016", "2017", "2018", "2019", "2020", "2021",
                           "2022", "2023", "2024"])
        ...
        flat_type= st.selectbox("Flat Type", ['3 ROOM', '4 ROOM', '5 ROOM', '2 ROOM', 'EXECUTIVE', '1 ROOM',
                                                        'MULTI-GENERATION'])



        flr_area_sqm= st.number_input("Floor Area sqm (Min: 31 / Max: 280")

        flat_model= st.selectbox("Flat Model", ['Improved', 'New Generation', 'Model A', 'Standard', 'Simplified',
    ...
    with col2:
        stry_start= st.number_input("Storey Start")

        stry_end= st.number_input("Storey End")

        re_les_year= st.number_input("Remaining Lease Year (Min: 42 / Max: 97)")

        re_les_month= st.number_input("Remaining Lease Month (Min: 0 / Max: 11)")

        les_coms_dt= st.selectbox("Lease_Commence_Date", [str(i) for i in range(1966,2023)])
    ...
    if button:
        ...
        st.write("## :green[**Predicted Resale Price is :**]",pre_price)
